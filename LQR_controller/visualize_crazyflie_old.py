...
def quadrotor_visualize(
    ...
    positions = np.asarray(
        [positions[k] for k in range(0, len(positions), traj_step)] + [positions[-1]]
    )
    N = positions.shape[0]
    trace_positions = []
    orientations = np.asarray(
        [orientations[k] for k in range(0, len(orientations), traj_step)]
        + [orientations[-1]]
    )
    ...
    body_orient0 = np.array(
        [
            [np.cos(body_yaw0), -np.sin(body_yaw0), 0.0],
    ...
    fig.canvas.mpl_connect(
        "key_release_event", lambda event: [exit(0) if event.key == "q" else None]
    )
    for k in range(N):
        r = positions[k]
        R = orientations[k]
        ...
        ax_world.set_xlim(
            x_min - ax_world_margin, x_min + ax_world_range + ax_world_margin
        )
        ax_world.set_ylim(
            y_min - ax_world_margin, y_min + ax_world_range + ax_world_margin
        )
        ax_world.set_zlim(0.0, z_max + ax_world_margin)
        # ax_world.set_aspect("equal")

        ax_world.scatter(
            trace_positions_array[:, 0],
            trace_positions_array[:, 1],
            trace_positions_array[:, 2],
            c="k",
            s=20,
            label="trace",
        )
        ...
        for i in range(4):
            ...
            ax_world.plot(
                [pB[0], pA[0]], [pB[1], pA[1]], [pB[2], pA[2]], "k-", linewidth=1
            )
            ax_world.plot(
                [pMs[i][0], pCs[i][0]],
                [pMs[i][1], pCs[i][1]],
        ...
        if cam_onboard:
            ...
            ax_body.scatter(
                pMs[:, 0], pMs[:, 1], pMs[:, 2], c="g", s=50, label="motors"
            )
            ...
            for i in range(4):
                ...
                text_pos = pCs[i]
                ax_body.text(
                    *text_pos, str(i + 1), fontsize=12, color="blue", ha="center"
                )
