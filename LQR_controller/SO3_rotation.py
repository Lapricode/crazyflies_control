import numpy as np
import LQR_controller.tolerances as tolerances
...
def log(R):
    ...
    if abs(theta) < tol:
        return (R - R.T) / 2.0
    tau_hat = theta * (R - R.T) / (2.0 * np.sin(theta))
    return tau_hat
...
def Exp(tau):
    theta, u = decompose_cartesian_element(tau)
    u_hat = vec_hat(u)
    R = np.eye(3) + np.sin(theta) * u_hat + (1.0 - np.cos(theta)) * u_hat @ u_hat
    return R
...
def Log(R):
    theta = np.arccos((np.trace(R) - 1.0) / 2.0)
    if abs(theta) < tol:
        return compose_cartesian_element(theta, np.array([0.0, 0.0, 1.0]))
    tau_hat = theta * (R - R.T) / (2.0 * np.sin(theta))
...
def jacobian_right_inverse(tau):
    ...
    return (
        np.eye(3)
        + 0.5 * theta * u_hat
        + (1.0 - 0.5 * theta * (1.0 + np.cos(theta)) / np.sin(theta)) * u_hat @ u_hat
        if abs(theta) >= tol
        else np.eye(3)
    )
...
def vec_hat(v):
    return np.block([[0.0, -v[2], v[1]], [v[2], 0.0, -v[0]], [-v[1], v[0], 0.0]])
