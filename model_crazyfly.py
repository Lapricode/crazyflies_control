...
def finite_differences(f, x, u, dt, eps = 1e-5):
    ...
    for i in range(N):
        ...
        if i == 3:
            print(compute_state_right_plus(x, ei))
            print(compute_state_right_plus(x, -ei))
            print(compute_state_right_minus(fx_plus, fx_minus))
...
# wrap your euler so its signature is (x, u, dt) → next x
